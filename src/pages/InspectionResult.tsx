...
export default function InspectionResult() {
  ...
  const handleSaveToOfficerLog = () => {
    ...
    saveOfficerInspection({
      ...
      status: (currentScore >= 80 ? 'Healthy' : currentScore >= 60 ? 'Attention' : 'At Risk'),
      securityHash: inspectionData.securityHash || 'SHA256:7f3a9e10c4b281d5',
      notes: inspectionData.description || 'Verified AI visual inspection audit.',
      diagnosticSummary: isGemini ? geminiData?.diagnosticSummary : 'Surface oxidation, micro-fractures, and structural fatigue evaluated.',
      defectsCount: isMachine ? 3 : 2,
      isGemini,
    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      {saveToast && (
        <div className="fixed top-20 right-6 z-50 bg-slate-900 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-xl flex items-center gap-2 animate-in fade-in slide-in-from-top-2 border border-emerald-500/30">
          <Check className="w-4 h-4 text-emerald-400" /> Audit record successfully saved to Officer Log!
        </div>
      )}
      ...
      <div className="flex flex-col items-center text-center space-y-3">
        ...
        {/* Action Buttons: Save Work + View Report */}
        <div className="flex flex-wrap items-center justify-center gap-3 mt-4">
          <button
            onClick={handleSaveToOfficerLog}
            className={`px-6 py-3 rounded-2xl text-xs font-extrabold transition-all shadow-md flex items-center gap-2 cursor-pointer ${
              isSaved
                ? 'bg-emerald-500/15 text-emerald-600 border border-emerald-500/30'
          ...
          <Link
            to="/report"
            className="px-6 py-3 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-extrabold shadow-md transition-all flex items-center gap-2 cursor-pointer"
          >
